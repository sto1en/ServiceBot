...
bot = telebot.TeleBot(applications.get_bot_api())
...
def start(message):
    ...
    markup.add(button_url, button_register, button_service)
...
def response(function_call):
    if function_call.message:
        ...
        #Личный кабинет
            ...
            client.id = function_call.from_user.id
            client.state[function_call.message.chat.id] = 'account'
            text = applications.get_client_info(client)
            markup = telebot.types.InlineKeyboardMarkup(row_width=2)
            if text == 'Вы не зарегистрированы!':
                ...
                button_record = telebot.types.InlineKeyboardButton(text="Ваши записи", callback_data="see_records")
                button_correct_info = telebot.types.InlineKeyboardButton(text='Изменить контактную информацию', callback_data='client_correct')
                button_delete_info = telebot.types.InlineKeyboardButton(text='Удалить контактную информацию', callback_data='client_delete')
                markup.add(button_record)
                markup.add(button_correct_info)
                markup.add(button_delete_info)
        ...
        #Запись на обслуживание
            ...
            if applications.check_record_in_table(record.date, record.time):
                applications.date_sign(record.date, record.time, client.id, record.info)
...
def AdminBot(message):
    admins = applications.get_admins()
    for i in range(len(admins)):
        if message.from_user.id == admins[i]:
...
def SuperAdminBot(message):
    admins = applications.get_admins()
    if message.from_user.id == admins[0] or message.from_user.id == admins[1]:
        admin_state[message.chat.id] = "menu"
        text = "Меню"
        markup = telebot.types.InlineKeyboardMarkup(row_width = 2)
